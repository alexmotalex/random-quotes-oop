...
1. Run server in the production mode:
   `npm start`
1. Configure hosting server where you run application to forward all request to the http://localhost:3000
1. Take assigned by the hosting provider URL for you backend API server.
...
1. In the `client/src/config.js` replace http://localhost:3000 with url assigned to the
...
1. Open https://random-quotes-frontend.com in web browser.
